...
Identify the reference's palette (pull actual hex values), type style, spacing rhythm,
and the one signature move that makes it feel like itself.
Match that direction. Do not "improve" it toward a generic look.
If the reference conflicts with a hard ban (e.g. it's purple), tell the user and ask
...
the code runs.
Screenshot / preview the page at desktop and mobile widths if the environment allows.
A picture is worth 1000 tokens.
Run the vibe-code checklist below. If any box is checked, fix it.
Read your own copy — does it sound like an interchangeable SaaS template? Rewrite it
to be specific and plain.
Click through key interactions (nav, mobile menu, buttons, forms) and confirm they work.
Apply Chanel's rule: look at the finished page and remove one thing that isn't earning its
place.
Report back what you checked and what you changed.
